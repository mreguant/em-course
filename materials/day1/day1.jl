...
Let's load the libraries we will use. Here we will be loading a bunch of libraries so that we can use Julia as if we were using Python with Python and PyPlots! It defeats a bit the purpose, but the graphing capabilities of Python are much more commonly used, so it is useful to use Python to do basic data work.
...
begin
	close("all")
	regplot(x="wind_forecast", y="emis_tCO2", data=df, x_bins=10)
...
begin
	close("all")
	regplot(x="wind_forecast", y="wholesale_price", data=df, x_bins=10)
...
begin
	close("all")
	regplot(x="wind_forecast", y="system_costs", data=df, x_bins=10)
	title("Wind increases congestion and reliability costs")
	xlabel("Wind forecast (GWh)")
...
begin
	df["full_price"] = df.wholesale_price + df.system_costs
	close("all")
	regplot(x="wind_forecast", y="full_price", data=df, x_bins=10)
...
git-tree-sha1 = "1f472e1dbf8518156f6425ba2a9f539ac188177f"
...
git-tree-sha1 = "13093d3b0f1475a20918a00675c4a3a13bcd164b"
